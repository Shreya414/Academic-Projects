...
class Product:
    ...
    def __init__(self,root):
        ...
        self.root = root
        self.root.title("WAREHOUSE SALES PURCHASE MANAGEMENT SYSTEM")
        ...
        '''Lets call the database methods to perform te database operations '''
        #function to close the Product frame    
        def close():
            print("Product : close method called")
            close = tkinter.messagebox.askyesno("WAREHOUSE SALES PURCHASE MANAGEMENT SYSTEM","Do you really want to close the system?")
            if close > 0:
                  root.destroy()
        ...
        def insert():
            ...
            if (len(pId.get())!=0):
                ...
                tkinter.messagebox.askyesno("WAREHOUSE SALES PURCHASE MANAGEMENT SYSTEM","Enter Product ID")
                print("Product : insert method Finished\n")

        #function responsible to product table data to scroll product list
        ...
        self.ITitle = Label(HeadFrame, font=('arial',40,'bold'), fg='red',text='Warehouse Inventory Sales Purchase ',bg='white')
...
class Database:
    ...
    def update(self,pid="",name="",price="",qty="",company="",contact=""):
        ...
        con = sqlite3.connect("inventory.db")
        cur = con.cursor()
        cur.execute("update product set pid=? or pname=? or price=? or qty=? or company=? or contact=? where pid=?", (pid,pname,price,qty,company,contact,pid))
        con.commit()
        con.close()
